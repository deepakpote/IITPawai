    ...
    ['$scope', '$location', '$log', '$window', '$state', '$http', '$uibModal', 'appUtils', 'appConstants', 'contentService', 'commonService', '$filter',
        function($scope, $location, $log, $window, $state, $http, $uibModal, appUtils, appConstants, contentService, commonService, filter) {
            ...
            $scope.sendForReview = function() {
                ...
                if(validateOptions()){
                    $scope.statusCodeID = appConstants.statusCode.sentForReview;
                    $log.debug("in sfr");
                    submit();
                }
            };
            ...
            var validateOptions = function() {
                if ($scope.content.fileTypeCodeID == appConstants.fileTypeCode.video){
                ...
                $http.post(appConstants.endpoint.baseUrl + "content/uploadContent/", fd, {
                    ...
                    .then (function success(response){
                            ...
                            modalInstance.result.finally(function(){
                                ...
                                else if ($scope.content.contentTypeCodeID == appConstants.contentTypeCode.selfLearning) {
                                    $window.scrollTo(0, 0);
            ...
            $scope.isContentTypeTeachingAid = function (response) {
                return ($scope.content.contentTypeCodeID === appConstants.contentTypeCode.teachingAids);
            };

            $scope.isContentTypeSelfLearning = function (response) {
                return ($scope.content.contentTypeCodeID === appConstants.contentTypeCode.selfLearning);
            };

            $scope.isFileTypeVideo = function (response) {
                return ($scope.content.fileTypeCodeID === appConstants.fileTypeCode.video);
            };
            ...
            var setSuccessDetails = function() {
                ...
                if ($scope.statusCodeID == appConstants.statusCode.created){

                    $scope.success.message = "Saved To Drafts";
                }
                else if ($scope.statusCodeID == appConstants.statusCode.sentForReview){
                ...
                if ($scope.content.fileTypeCodeID == appConstants.fileTypeCode.video && $scope.inputs.fileName){
